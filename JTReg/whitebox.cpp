...
class OldRegionsStatClosure: public HeapRegionClosure {
 ...
 private:
  ...
  size_t _total_used;
  size_t _reg_size;
  size_t _total_could_free_region;
  size_t _total_memory_to_free;
  

 public:
  OldRegionsStatClosure(bool print_flag) :
    _print_flag(print_flag),
    _total_live(0),
    _total_count(0),
    _total_used(0),
    _reg_size(HeapRegion::GrainBytes),
    _total_could_free_region(0),
    _total_memory_to_free(0) { }
   

  size_t total_live() { return _total_live; }
  size_t total_count() { return _total_count; }
  size_t total_used() { return _total_used; }
  size_t total_could_free_region() { return _total_could_free_region; }
  size_t total_memory_to_free() { return _total_memory_to_free; }
  size_t reg_size() { return _reg_size; }
...
WB_ENTRY(jlongArray, WB_G1GetOldStat(JNIEnv* env, jobject o, jboolean printFlag))
  if (!UseG1GC) {
    THROW_MSG_NULL(vmSymbols::java_lang_UnsupportedOperationException(), "WB_G1GetOldStat: G1 GC is not enabled");
  }
  

  G1CollectedHeap* g1h = G1CollectedHeap::heap();
  OldRegionsStatClosure rs(printFlag);
  g1h->heap_region_iterate(&rs);

  typeArrayOop result = oopFactory::new_longArray(6, CHECK_NULL);
  result->long_at_put(0, rs.total_count());
  result->long_at_put(1, rs.total_used());
  result->long_at_put(2, rs.total_live());
  result->long_at_put(3, rs.total_could_free_region());
  result->long_at_put(4, rs.total_memory_to_free());
  result->long_at_put(5, rs.reg_size());
...
static JNINativeMethod methods[] = {
  ...
  {CC"g1GetOldStat",   CC"(Z)[J",                 (void*)&WB_G1GetOldStat },
#endif // INCLUDE_G1GC
#if INCLUDE_PARALLELGC
