...
以全部开启动态调整参数的情况下和完全关闭为例，其中的一些性能指标如下：

|JDK | Throughput | Full GC次数| Mixed GC次数 | 平均Mixed GC时间 （ms)| GC 吞吐量(%) |
|---|----------|---------------|---------------|---------------|-----------|
|AdaptMixedGC| 2257013   | 32          | 412          | 37.1         | 82.1      |
|Default| 2196318   | 56          | 402          | 32.0        | 81.2      |

其中动态调整参数能够获得一定程度上的优化，Full GC次数能够明显下降，同时Mixed GC平均时间的上升也与我们参数调整的预想相符合，但注意到与我们手动调整较好的参数效果仍有一定差距。此外因为该benchmark的只是简单的toy benchmark，
有很大的不稳定性，并且只有堆内存上升的趋势，无法验证能否当GC足够快时调整以节省计算资源的能力，所以我们需要更多的测试来验证该方法的有效性。
